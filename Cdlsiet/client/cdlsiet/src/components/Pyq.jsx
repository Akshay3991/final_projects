...
const Pyq = () => {
  ...
  const { PyqList, addPyqList } = useContext(PostList);
  // console.log(PyqList)
  useEffect(() => {
    async function getpyq() {
      await fetch('https://cdlsiet-server.onrender.com/cdlsiet/pyq')
        .then((res) => res.json())
        .then((data) => {
          // console.log(data)
          addPyqList(data)
  ...
  let pyqitems = PyqList.map((data) => {
    return (
      <PyqItems key={data.id} data={data} />
    )
  })
  return (
